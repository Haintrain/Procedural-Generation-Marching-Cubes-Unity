...
public class VoxelMap : MonoBehaviour {
    ...
    public void EditVoxelsGenerate (Vector3 point, bool filledIs) {
        int centerX = (int)point.x;
        int centerY = (int)point.y;

        centerX = centerX / voxelResolution;
        centerY = centerY / voxelResolution;

        int i = centerY + centerX * chunkResolution;
        chunks[i].Apply(point, filledIs);
	}

    public void Refresh(){
        for(int i = 0; i < Mathf.Pow(chunkResolution, 2); i++){
            chunks[i].Refresh();
        }
    }
